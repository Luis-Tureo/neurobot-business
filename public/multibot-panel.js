...
function renderModerationGroups(groups) {
  ...
  groups.forEach((group) => {
    ...
    select.value=group.mode; select.addEventListener('change',async()=>{
      try { await panelApi(`/api/bots/${encodeURIComponent(panelState.selectedBotId)}/moderation/groups/${encodeURIComponent(group.groupHash)}`,{method:'PATCH',body:JSON.stringify({mode:select.value})}); await Promise.resolve(); notify('Moderación del grupo actualizada.'); }
...
function renderModerationRules(rules) {
  ...
  rules.forEach((rule)=>{
    ...
    actions.append(actionButton('Editar','secondary',()=>editModerationRule(rule)),actionButton(rule.enabled?'Desactivar':'Activar','secondary',async()=>{
      await panelApi(`/api/bots/${encodeURIComponent(panelState.selectedBotId)}/moderation/rules/${rule.id}`,{method:'PUT',body:JSON.stringify({...moderationRulePayload(rule),enabled:!rule.enabled})}); await Promise.resolve();
    }),actionButton('Eliminar','danger',async()=>{if(!window.confirm('¿Eliminar esta regla?'))return;await panelApi(`/api/bots/${encodeURIComponent(panelState.selectedBotId)}/moderation/rules/${rule.id}`,{method:'DELETE'});await Promise.resolve();}));
...
function renderModerationTerms(terms) {
  ...
  terms.forEach((term)=>{const item=createListItem(term.term,`${term.category} · ${term.severity} · ${term.matchMode}`);item.append(actionButton('Eliminar','danger',async()=>{await panelApi(`/api/bots/${encodeURIComponent(panelState.selectedBotId)}/moderation/terms/${term.id}`,{method:'DELETE'});await Promise.resolve();}));target.append(item);});
}
...
async function loadBotSummary(refreshForms = true) {
  ...
  const cards = [
    ...
    ['Modo', botModeLabel(result.bot.mode)],
...
function configureForms() {
  ...
  document.querySelector('#moderation-settings-form').addEventListener('submit',async(event)=>{
    ...
    try{await panelApi(`/api/bots/${encodeURIComponent(panelState.selectedBotId)}/moderation/settings`,{method:'PATCH',body:JSON.stringify(payload)});await Promise.resolve();notify('Configuración de moderación guardada.');}catch(error){notify(error.message,true);}
  });
  document.querySelector('#moderation-warning-form').addEventListener('submit',async(event)=>{
    ...
    try{await panelApi(`/api/bots/${encodeURIComponent(panelState.selectedBotId)}/moderation/settings`,{method:'PATCH',body:JSON.stringify(settings)});await Promise.resolve();notify('Mensajes de advertencia guardados.');}catch(error){notify(error.message,true);}
  });
  ...
  document.querySelector('#moderation-term-form').addEventListener('submit',async(event)=>{
    ...
    try{await panelApi(`/api/bots/${encodeURIComponent(panelState.selectedBotId)}/moderation/terms`,{method:'POST',body:JSON.stringify(payload)});form.reset();form.elements.category.value='RESPETO';form.elements.score.value=1;await Promise.resolve();notify('Término agregado.');}catch(error){notify(error.message,true);}
  });
