...
function applyBotModules(modules = []) {
  const businessModules = modules.filter(
    (module) => !['automatic-messages', 'polls', 'moderation'].includes(module),
  );
...
function setGlobalContext(section = 'bots', activate = true) {
  ...
  document.querySelector('#application-title').textContent = 'Neurobot Business';
  document.querySelector('#application-subtitle').textContent =
    'Administra la atención automática y humana de cada negocio.';
...
async function loadBots() {
  ...
  result.bots.forEach((bot) => {
    ...
    info.append(node('p', `Número: ${phoneText} • Estado: ${statusText}`, 'muted'));
...
function fillBotConfiguration(bot) {
  ...
  form.elements.realMentionRequired.checked = false;
  form.elements.realMentionRequired.disabled = true;
  ...
  document.querySelector('#community-menu-help')?.classList.add('hidden');
  document.querySelector('#community-single-turn-settings')?.classList.add('hidden');
}
