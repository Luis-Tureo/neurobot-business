...
describe('cola de solicitudes de IA por asistente', () => {
  ...
  beforeEach(() => {
    ...
    database.migrate();
    database.saveAIQueueSettings('negocio-ejemplo', {
      ...database.getAIQueueSettings('negocio-ejemplo'),
  ...
  it('envía un aviso una vez y expira sin ejecutar una consulta antigua', async () => {
    ...
    const queue = new AIRequestQueueService(database, createLogger('silent'), 'negocio-ejemplo');
  ...
  it('mantiene colas independientes por asistente', async () => {
    const firstBusiness = new AIRequestQueueService(
      database,
      createLogger('silent'),
      'negocio-ejemplo',
    );
    const otherBot = database.createBot({
      ...
      profile: database.getBotProfile('negocio-ejemplo'),
    });
    const business = new AIRequestQueueService(database, createLogger('silent'), otherBot.id);
    expect(firstBusiness).not.toBe(business);
    expect(firstBusiness.snapshot().settings).toMatchObject({ maxConcurrent: 3 });
...
describe('cola de salida por chat', () => {
  it('mantiene orden por chat sin mezclar colas', async () => {
    ...
    try {
      await Promise.all([
        outbound.send('56911111111', 'primero'),
        outbound.send('56911111111', 'segundo'),
        outbound.send('56922222222', 'independiente'),
      ]);
      expect(
        client.sentMessages
          .filter((item) => item.chatId === '56911111111')
          .map((item) => item.text),
      ).toEqual(['primero', 'segundo']);
      expect(client.sentMessages.filter((item) => item.chatId === '56922222222')).toHaveLength(1);
    } finally {
      database.close();
    }
  });
