...
describe('API administrativa', () => {
  ...
  it('expone un historial seguro, aislado por asistente y ordenado por fecha', async () => {
    database.recordTechnicalEvent({
      botId: 'neurobot',
      eventType: 'AI_CALL_SUCCESS',
      source: 'private',
      userHash: 'usuario-anonimo',
      commandName: 'campo-interno-no-expuesto',
      result: 'ok',
      durationMs: 25,
    });
    database.recordTechnicalEvent({
      botId: 'otro-asistente',
    ...
    const response = await app.inject({
      method: 'GET',
      url: '/api/bots/neurobot/history?limit=20',
      headers: { cookie: auth.cookie },
    });
    ...
    expect(response.body).not.toContain('campo-interno-no-expuesto');
