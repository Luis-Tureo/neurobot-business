...
replaceInFile('src/admin/server.ts', (c) => {
  ...
  // Fix TS18048 context.multiBotManager might be undefined... wait, in server.ts:
  // qrCode: context.multiBotManager.qr(bot.id) !== null,
...
replaceInFile('src/persistence/database.ts', (c) => {
  ...
  // Remove public getPollTemplates() method entirely
  c = lines.join('\n');
...
replaceInFile('src/domain/types.ts', (c) => {
  let lines = c.split('\n');
  lines = lines.filter(line => !line.includes('PollDeliverySource') && !line.includes('PollDeliveryStatus'));
  return lines.join('\n');
});
