...
function isBad(name) {
  ...
  return lower.includes('poll') || lower.includes('moderation') || lower.includes('groupdiscovery') || lower.includes('automaticmessage') || lower.includes('communityservices');
}
    ...
    ImportDeclaration(path) {
      ...
      if (src.includes('poll') || src.includes('group-discovery') || src.includes('moderation') || src.includes('automatic-message')) {
        ...
        path.node.specifiers = path.node.specifiers.filter(spec => {
          ...
          if (name && (isBad(name) || name.includes('GroupListSource') || name.includes('NativePoll'))) return false;
          return true;
        });
