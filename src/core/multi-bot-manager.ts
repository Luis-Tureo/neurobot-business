...
export class MultiBotManager {
  ...
  public constructor(private readonly database: AppDatabase, private readonly providers: AIProviderFactory, private readonly sessions: WhatsAppSessionManager, private readonly anonymizer: Anonymizer, private readonly logger: Logger, private readonly options: BotInstanceOptions & {
  ...
  public async prepare(botId: string): Promise<BotInstance> {
    ...
    if (!this.canStart(bot)) {
      throw new Error(bot.connectorType === 'WHATSAPP_CLOUD_API' ? 'El conector Cloud API queda pendiente hasta completar sus credenciales y webhook.' : 'El asistente no puede iniciarse en su estado actual.');
    }
    await this.sessions.pathFor(bot);
    bot = this.database.getBot(botId) as BotRecord;
    const instance = new BotInstance(bot, this.clientFactory(bot), this.database, this.providers.forBot(bot.id), this.anonymizer, this.logger, {
      ...this.options,
  ...
  public async create(): Promise<BotRecord> {
    this.database.recordTechnicalEvent({
      botId: bot.id,
  ...
  private recordInstanceFailure(botId: string, error: unknown): void {
  ...
  private canStart(bot: BotRecord): boolean {
    return bot.enabled && bot.connectorType === 'WHATSAPP_WEB' && !['ARCHIVED', 'PENDING_DELETION', 'DELETED', 'DUPLICATE_CONFIGURATION', 'DISABLED'].includes(bot.lifecycleStatus);
  }
}