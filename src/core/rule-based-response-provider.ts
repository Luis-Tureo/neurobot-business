...
export class RuleBasedResponseProvider implements ResponseProvider {
  ...
  public select(request: ResponseRequest): SelectedResponse | null {
    ...
    const match = this.database.listKeywords().filter(keyword => keyword.enabled && containsWholeTerm(request.text, keyword.term)).sort((left, right) => right.priority - left.priority).find(keyword => commands.get(keyword.commandId)?.enabled === true);
  ...
  private fallback(): SelectedResponse {
    return {
      text: this.database.getSetting('fallback_response', 'No encontré una respuesta configurada. Escribe !ayuda para ver las opciones.'),
      commandName: null
    };
  }
  private withWarning(text: string, healthRelated: boolean): string {
    if (!healthRelated) return text;
    const warning = this.database.getSetting('professional_warning', 'Esta información es solamente una orientación general y no reemplaza una evaluación médica, psicológica o profesional.');
    return `${text}\n\n${warning}`;
  }
}